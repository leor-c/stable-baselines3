...
class BaseModel(nn.Module, ABC):
    ...
    def extract_features(self, obs: th.Tensor) -> th.Tensor:
        ...
        if isinstance(obs, tuple):
            assert len(obs) == 2, "LSTM observation should be a 2-tuple of (obs, (h, c))."
            preprocessed_obs = preprocess_obs(obs[0], self.observation_space, normalize_images=self.normalize_images)
            return self.features_extractor(preprocessed_obs, obs[1])
        else:
...
class RecurrentActorCriticPolicy(ActorCriticPolicy):
    def __init__(self, observation_space: gym.spaces.Space, action_space: gym.spaces.Space, lr_schedule: Schedule,
                 net_arch: Optional[List[Union[int, Dict[str, List[int]]]]] = None,
                 activation_fn: Type[nn.Module] = nn.Tanh, ortho_init: bool = True, use_sde: bool = False,
                 log_std_init: float = 0.0, full_std: bool = True, sde_net_arch: Optional[List[int]] = None,
